...
mail_settings = {
    ...
    "MAIL_USE_TLS": False,
    "MAIL_USE_SSL": True,
    "MAIL_USERNAME": "mohammedmuzakeer23@gmail.com",
    "MAIL_PASSWORD": "ebbovizovlyjdkho"
}
...
def add_photo2():
    ...
    if request.method=='POST':
        ...
        try:
            ...
            dn1 = cursor.fetchone()[0]
            if dn1>0:
                cursor.execute("SELECT * FROM vt_face where vid!=%s",(vid,))
                dg1 = cursor.fetchall()
                cutoff=3
                for dg2 in dg1:
                    hash0 = imagehash.average_hash(Image.open("static/frame/"+dg2[2])) 
    ...
    return render_template('add_photo2.html',data=data, vid=vid,msg=msg)
...
def view_cus():
    ...
    if act=="delall":
        mycursor.execute("delete from vt_face")
        mydb.commit()
        mycursor.execute("delete from register")
        mydb.commit()
        mycursor.execute("delete from attendance")
        mydb.commit()
        for fname in os.listdir("static/frame"):
            if fname=="":
                s=1
            else:
                os.remove("static/frame/"+fname)
...
def process():
            ...
            if mcnt1>=2:
                ...
                for rr in dt:
                    ...
                    if cc1<=cutoff:
                        ...
                        rw = cursor.fetchone()
                        regno=rw[13]
                        msg="Employee: "+rw[1]+" ("+regno+"), Present "
                        mst=rr[3]
                        ...
                        cursor.execute("update attendance set attendance='Present',mask_st=%s,lat=%s,lon=%s where regno=%s",(det,lat,lon,regno))
                        mydb.commit()                    
                        break
                    else:
                        msg="Unknown"
    ...
    return render_template('process.html',msg=msg,act=act,mcnt1=mcnt1,det=det)
    ...
    if request.method=='POST':
        ...
        if rd2=="":
            ...
            if cnt>0:
                ...
                cursor.execute('SELECT * FROM attendance a,register r where a.regno=r.regno && (a.id>=%s or a.id<=%s)',(d1,d2))
                data = cursor.fetchall()

        
        

        cursor.execute("SELECT id,regno,rdate,attendance FROM attendance")
